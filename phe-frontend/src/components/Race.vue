  ...
  <b-container>
    <b-row>
      ...
      <b-col cols="8">Liste des paris :
        <b-list-group>
          <pari v-on:deletePari="deletePari" v-bind:paris="paris" v-bind:currentAuthor="curAuthor"></pari>
        </b-list-group>
      </b-col>
    </b-row>
    <hr>
...
export default {
  ...
  methods: {
    addPari: function(event) {
      ...
      axios
        .post('http://localhost:3000/paris', {author: this.curAuthor, horse: this.form.horse})
        .then(_ => {
          this.loadParis();
        })
        .catch(e => {
